...
section("doc-xdsinp", "Editing XDS.INP",
    ...
    UL([B("Load from XDS.INP") + " reads the project's file into the form; " + B("Load from other XDS.INP&hellip;") + " imports the parameters of any XDS.INP (beamline pipelines, previous runs) into the form only &mdash; press " + B("Save Parameters") + " to write them. In a project that has no XDS.INP yet, that first save keeps the whole loaded file (detector axes, rotation axis and the other keywords the form does not show) with the form's values on top.",
        ...
        B("Manual entry") + " in the Key Parameters form, or the raw text in the Full Input File tab. Parameters typed into a project without XDS.INP are saved with the standard geometry that Generate from images writes (check ROTATION_AXIS for your beamline)."]),
    SUB("The Key Parameters form"),
    P("Groups: experiment (oscillation, wavelength, template, distance), detector (type, NX/NY, ORGX/ORGY, QX/QY), indexing (space group, cell, SIGNAL_PIXEL, ranges), corrections, reporting (resolution range, shells) and the neggia " + C("LIB=") + " line. Next to LIB, " + B("from loaded XDS.INP") + " (unticked by default) decides whether loading an XDS.INP puts that file's " + C("LIB=") + " path in the field; unticked, the field keeps the HDF5 library set in the Environment screen, and a note names the path the file had. " + B("&#8634;") + " puts the Environment screen's library back in the field. Toggles mark a keyword " + B("Active") + " or " + B("Commented") + " (written with a leading " + C("!") + ", so the value is kept but XDS ignores it &mdash; set SPACE_GROUP_NUMBER to commented or 0 to let XDS decide)."),
    UL([B("From IDXREF") + " under the cell fields opens the lattice table of the latest IDXREF.LP; pick a Bravais lattice to fill cell and space group.",
        ...
        "These one-keyword buttons (cut-off, ice rings, space group) change an existing XDS.INP; in a project without one they say so."]),
    SUB("What CrystalPilot tidies before a program reads its file"),
    P("Just before XDS, XSCALE or XDSCONV starts, and whenever CrystalPilot writes XDS.INP, XSCALE.INP or XDSCONV.INP, three things those programs cannot read are corrected without changing a value: "
      "the byte-order mark Windows Notepad can put at the start of a file; in XDS.INP, a whole number written as a decimal in an integer keyword (" + C("NX= 4150.0") + " becomes " + C("NX= 4150") + "; "
      "also NY, OVERLOAD, DATA_RANGE, SPOT_RANGE, BACKGROUND_RANGE, EXCLUDE_DATA_RANGE, SPACE_GROUP_NUMBER and the like); and " + C("STRONG_PIXEL=") + ", found in older beamline files and no longer a keyword of XDS, "
      "which is written as " + C("SIGNAL_PIXEL=") + " with the same value and a " + C("! CrystalPilot:") + " note (commented out instead when the file already sets SIGNAL_PIXEL)."),
...
section("doc-xscale", "XSCALE",
    ...
    UL([B("Update in existing XSCALE.INP") + " merges the form into the file and keeps everything else, comments included.",
        ...
        "Opening a project fills the form from that project's XSCALE.INP, or with the defaults when it has none, so input files of another project are never saved into it.",
...
section("doc-xdsconv", "XDSCONV and MTZ",
    ...
    UL(["With CCP4 available, " + C("f2mtz") + " and " + C("cad") + " run after XDSCONV and produce the final " + C(".mtz") + " (custom name possible). On Windows this uses CCP4 for Windows through the bridge installed by the wizard.",
        ...
        "Opening a project fills the form from its XDSCONV.INP, or with the defaults when it has none. " + B("Save") + " fills an empty input file with the OUTPUT_FILE of XSCALE and an empty output name with the one derived from the project.",
        "A run that XDSCONV stops with an error (for example a wrong input file) is shown as failed."]),
)
