...
section("doc-pipeline", "Running XDS",
    ...
    UL(["Before each run the project's XDS.INP is copied into the run folder, so what you see in the editor is what runs.",
        ...
        B("CPU cores") + " and " + B("RAM (GB)") + " (sidebar, XDS Config) limit what XDS, XSCALE and the other programs may use, all runs together, so the computer stays usable: 4 cores and 16 GB (at most three quarters of the RAM) by default. The core count is written into the run's XDS.INP / XSCALE.INP as " + C("MAXIMUM_NUMBER_OF_PROCESSORS") + " (with " + C("MAXIMUM_NUMBER_OF_JOBS= 1") + ") and the programs are held to that many cores. The RAM limit uses a Linux cgroup (as root, e.g. under WSL) or a systemd user scope; it counts the frames Linux keeps in memory too, which are dropped first. A run that needs more RAM than allowed is stopped and the log says so. Where neither is available the panel says why, and only the CPU limit applies.",
