...
def stream_xdsconv(project_name, write_fn, work_dir=None, mtz_name=None):
    ...
    if status == "completed" and CCP4_BIN:
        ...
        if f2mtz_inp.exists() and _output_stamp(f2mtz_inp) != f2mtz_before:
            f2mtz_exe = Path(CCP4_BIN) / "f2mtz"
            cad_exe = Path(CCP4_BIN) / "cad"
            if f2mtz_exe.exists() and cad_exe.exists():
                ...
                try:
                    with open(str(f2mtz_inp), 'r', encoding='utf-8') as fin:
                        # Build CCP4 environment for subprocess
                        env = os.environ.copy()
                        ccp4_root = str(Path(CCP4_BIN).parent)
                        env.setdefault("CCP4", ccp4_root)
                        env.setdefault("CLIBD", str(Path(ccp4_root) / "lib" / "data"))
                        env.setdefault("CCP4_SCR", str(work_dir))
                        env.setdefault("CINCL", str(Path(ccp4_root) / "include"))
                        env["PATH"] = CCP4_BIN + os.pathsep + env.get("PATH", "")

                        _run_checked([str(f2mtz_exe), "HKLOUT", str(temp_mtz)], work_dir,
                ...
                try:
                    ...
                    _run_checked([str(cad_exe), "HKLIN1", str(temp_mtz), "HKLOUT", str(final_mtz)], work_dir,
                                   lambda t: send("log", {"text": t}), timeout=600, env=env,
                ...
                send("log", {"text": ">>> CCP4 f2mtz/cad not found - skipping MTZ conversion"})
        else:
...
def stream_pointless(project_name, write_fn, input_file=None, chirality="CHIRAL",
    ...
    ptl_exe = Path(CCP4_BIN) / "pointless"
    if not ptl_exe.exists():
        send("error_msg", {"message": "pointless not found in " + CCP4_BIN + ". Ensure CCP4 is installed correctly."})
        send("done", {})
        return
    ...
    # Build CCP4 environment
    env = os.environ.copy()
    ccp4_root = str(Path(CCP4_BIN).parent)
    env.setdefault("CCP4", ccp4_root)
    env.setdefault("CLIBD", str(Path(ccp4_root) / "lib" / "data"))
    env.setdefault("CCP4_SCR", str(_project_out_dir(project_dir)))
    env.setdefault("CINCL", str(Path(ccp4_root) / "include"))
    env["PATH"] = CCP4_BIN + os.pathsep + env.get("PATH", "")
...
def stream_aimless(project_name, write_fn, resolution_low=None, resolution_high=None,
    ...
    aimless_exe = Path(CCP4_BIN) / "aimless"
    if not aimless_exe.exists():
        send("error_msg", {"message": "aimless not found in " + CCP4_BIN + ". Ensure CCP4 is installed correctly."})
        send("done", {})
        return

    ptl_exe = Path(CCP4_BIN) / "pointless"
    if not ptl_exe.exists():
        send("error_msg", {"message": "pointless not found in " + CCP4_BIN + ". POINTLESS is required to prepare input for AIMLESS."})
        send("done", {})
        return

    # ── CCP4 environment (reused for all steps) ──
    env = os.environ.copy()
    ccp4_root = str(Path(CCP4_BIN).parent)
    env.setdefault("CCP4", ccp4_root)
    env.setdefault("CLIBD", str(Path(ccp4_root) / "lib" / "data"))
    ...
    if run_ctruncate and aimless_mtz.exists():
        ctr_exe = Path(CCP4_BIN) / "ctruncate"
        if not ctr_exe.exists():
            send("log", {"text": ">>> ctruncate not found in " + CCP4_BIN + " — skipping."})
        else:
    ...
    try:
        ...
        for name in _AP_INTEGRATION_FILES:
            ...
            if src.is_file():
                ...
                copied += 1
        send("ap_log", {"text": ">>> First integration put aside (" + str(copied) + " files) in case re-integration is worse"})
        return snap
...
def stream_autopilot(project_name, write_fn, criterion='isig2', friedel='FALSE', template=None, optimize=True, dcc_half=True, neggia_lib='',
    ...
    if optimize:
        ...
        # ── 'if_better': keep the first integration within reach ───────
        pre_optimize_dir = None
        if optimize == 'if_better':
        ...
        if optimize and not _job_stopped():
            ...
            if cor_lp_opt and not cor_err_opt:
                ...
                new_ice = [r for r in ice_rings_opt if r.get('detected')]
        ...
        if xdsconv_exe.exists():
            ...
            if conversion_ok and f2mtz_inp.exists():
                ...
                if f2mtz_exe.exists() and cad_exe.exists():
                    ...
                    try:
                        with open(str(f2mtz_inp), 'r', encoding='utf-8') as fin:
                            _run_checked([str(f2mtz_exe), "HKLOUT", str(temp_mtz)], _project_out_dir(project_dir),
                    ...
                    if f2mtz_ok and temp_mtz.exists():
                        ...
                        try:
                            _run_checked([str(cad_exe), "HKLIN1", str(temp_mtz), "HKLOUT", str(final_mtz)],
                                           _project_out_dir(project_dir), lambda t: None, timeout=600, env=env,
                                           stdin_text="LABIN FILE 1 ALL\nEND\n",
                    ...
                    send("ap_log", {"text": ">>> f2mtz/cad not found in CCP4"})
            else:
